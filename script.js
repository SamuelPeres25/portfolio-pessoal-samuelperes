...
const cards = documento.querySelectorAll(".container-testimnonials > div");
...
function showCards(){
    cards.forEach((card, index) => {
        if(index >= currentIndex && index < currentIndex + getVisibleCardCount()){
            ...
            card.syle.display = "none";
        }
    });

    const disablePrevbutton = currentIndex === 0;

    disablePrevbutton ? prevButton.classList.add('disabled') : prevButton.classList.remove('disabled');
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    form.addEventListener("submit", function(e){
        ...
        fetch("https://nodemailer-samuelperes.onrender.com/send-email", {
            method: "POST",
            headers: {
            ...
            if(res.ok){
                ...
                console.error(`Erro na resposta da api: ${res.status} - ${res.stuatusText}`)
            }
        }).catch((error)=> {
            console.error(error);
            loading.style.display = "none";
